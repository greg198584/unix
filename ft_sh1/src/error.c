...
/*   Updated: 2014/12/29 14:13:07 by glafitte         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	ft_env_error(char *env, char type, int pos)
{
	...
	else if (type == 1)
	{
		...
		ft_putstr_fd("env: cannot unset '': Invalid argument\n", 2);
	if (type != 3)
...
{
	ft_printf("If %s", cmd);
	ft_putstr_fd("' is not a typo you can use command-not-found to lookup", 2);
	ft_putstr_fd(" the package that contains it, like this:\n\tconf ", 2);
	ft_putendl_fd(cmd, 2);
	return (-1);
}
