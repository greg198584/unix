...
/*   Updated: 2014/12/27 16:53:57 by glafitte         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char		**ft_conv_lst(t_env *env)
{
	...
	if ((arr = malloc(sizeof(char *) * (count + 1))) == NULL)
		ft_puterror("error: problem to malloc in my_exec.c: l30\n");
	i = -1;
	tmp = env->next;
	while (tmp != env)
	{
...
static char	*ft_pathfile(char *path, char *exec)
{
	...
	if ((res = malloc(sizeof(char) * (size + ft_strlen(exec) + 1))) == NULL)
		ft_puterror("error: problem to malloc in my_exec.c: l56\n");
	res[0] = '\0';
...
static char	*ft_access(char *str, char *path)
{
	...
	arr = ft_str_to_wordtab(path, ':', ':');
	i = -1;
	while (arr[++i] != NULL)
	{
		exec = ft_pathfile(arr[i], str);
...
char	ft_exec(t_env *env, char **exec_opt, char *path)
{
	...
	if (pid == -1)
	{
		ft_error("impossible to find the son for fork!\n", "fork");
		return (1);
	}
	else if (pid == 0)
	{
		...
		{
			ft_free_arr(env_local);
			return (ft_cnf((exec == NULL) ? exec_opt[0] : exec));
		}
	}
	else
		wait(&statut);
